...
def get_estimated_slope(temp, temp_err, visc, visc_err, radius, radius_error):
    """
    ...
    :param radius_error: the error in particle radius, in meters
    :return: a tupple in the form (slope, slope_err) such that slope_err is the propagated error. Both return
    values are in m^2/s
    """
    slope = (2 * const.k * temp) / (3 * const.pi * visc * radius)
    slope_err = sqrt((temp_err / temp) ^ 2 + (visc_err / visc) ^ 2 + (radius_error / radius) ^ 2) * slope
